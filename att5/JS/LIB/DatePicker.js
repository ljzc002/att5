...
*/
var datepicker_adjustx=0;
var datepicker_adjusty=0;
/**   
 * Date Picker
 ...
 * @param   id 		   要将datepicker填入的标签ID
 */
function setday(inputObj,dateFormatStyle,beginDate,endDate,lang,type,id,x,y){
	if(null == inputObj){return null;}
	datepicker_adjustx=x?x:0;//强行对日历的位置进行调整
	datepicker_adjusty=y?y:0;
	new Calendar(inputObj,dateFormatStyle,beginDate,endDate,lang,type,id).show();
}
...
Calendar.prototype.style = function(){
	...
	strStyle += ".calendar {font-size:12px; margin:0;padding:0px;border:1px solid #99cccc;background-color:#DBE7F2;}";
	...
	strStyle += ".calendar li a.hasArticle {font-weight:bold; color:#f06 !important}";
	...
	strStyle += ".today{ background-color:#ffffaa;border:1px solid #f06;padding:0 1px;}";
 ...
 * @update 
 */   
function Calendar(inputObj, dateFormatStyle, beginDate, endDate, lang, type,id) {
  ...
  this.dateControl = inputObj;
	//document.getElementById("bottom_table").innerHTML="<div id='calendarPanel' style='position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;font-size:12px;width:20px;'><iframe id='calendarIframe' scrolling='no' frameborder='0' width='100%' height='100%'></iframe></div>";
	//var bottom_table=document.getElementById("bottom_table");
	var div1=document.createElement("div");
	var iframe1=document.createElement("iframe");
	iframe1.setAttribute("id","calendarIframe");
	iframe1.setAttribute("scrolling","no");
	iframe1.setAttribute("frameborder","0");
	iframe1.setAttribute("width","100%");
	iframe1.setAttribute("height","162px");
	div1.appendChild(iframe1);
	div1.setAttribute("id","calendarPanel");

	div1.setAttribute("style","position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;font-size:12px;width:20px;height:162px;margin:0");
	div1.style.visibility="hidden";
	//document.getElementById("div_topbtn").appendChild(div1);//这里写把日历div加到哪个区域中
	//con_getElementsByClassName("div_topbtn")[tab_no_now].appendChild(div1);
  ...
  if("d" == this.type){
	...
	for (i = 1; i <= thisMonthEndDate; i++) { // Current Month's Date
		...
		if(currentDate.format("yyyy-MM-dd") == (this.date).format("yyyy-MM-dd")){
			//lis += "class='today' ";//@@在IE嵌套iframe中这里引发了焦点异常
		}
		lis += ">" + i + "</a></li>";
	}
	var j = 1;
...
Calendar.prototype.bingEvent = function(){
  ...
  if(this.getElementById("PrevMonth")){
	  this.getElementById("PrevMonth").onclick = function(){
	  	...
	  	calendar.draw();
		  calendar.date = calendar.date.dateAdd("m",0);
		  calendar.draw();
	  }
...
Calendar.prototype.show = function () {
  ...
  this.panel.style.left = xy.x+3 + "px";
  this.panel.style.top = (xy.y + this.dateControl.offsetHeight+4+datepicker_adjusty) + "px";
...
Calendar.prototype.hide = function() {
  ...
  this.isFocus = false;
	try{
		HandleFatapickerFocus(this.panel.parentNode);
	}catch(e)
	{
	}
	//this.panel.parentNode.focus();
}
...
/*document.getElementById("bottom_table").innerHTML('<div id="calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;font-size:12px;width:20px;">');
document.getElementById("bottom_table").innerHTML("<iframe id='calendarIframe' scrolling='no' frameborder='0' width='100%' height='100%'></iframe></div>");*/